...
try:
    from agent.sources import (
        ...
        # Layer 1 diversity: cross-reference sources beyond LinkedIn
...
TOOLS: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "description": (
            "Search Wellfound (AngelList) public job listings for company and role signals. "
            "No login, no API key. Good for startup/VC-backed company discovery."
        ),
        "parameters": {
            "type": "object",
            "properties": {
                "query": {"type": "string", "description": "ICP or role keyword to search."},
    ...
    },
    {
        ...
        "description": (
            "Fetch Crunchbase News RSS for public funding announcements, launches, and acquisitions. "
            "No API key required. Returns company-level funding signals."
        ),
        "parameters": {
            "type": "object",
            "properties": {
                "query": {"type": "string", "description": "Company or category keyword."},
                "limit": {"type": "integer", "description": "Max results (default 20)."}
            },
            "required": ["query"]
        }
    },
    {
        ...
        "description": (
            "Scrape a company's public About/Team page for self-published employee bios. "
            "Extracts name and title only. Company-own website data — NOT LinkedIn scraping. "
            "Requires company_name and domain."
        ),
        "parameters": {
            "type": "object",
            "properties": {
                ...
                "limit": {"type": "integer", "description": "Max people to extract (default 10)."}
            },
    ...
    },
    {
        ...
        "description": (
            "Search Sessionize for public conference speaker profiles. "
            "Self-published, opt-in public data. Good for finding ICP prospects "
            "who are actively visible in their industry."
        ),
        "parameters": {
            "type": "object",
            "properties": {
                "query": {"type": "string", "description": "Topic, title, or ICP keyword."},
                "limit": {"type": "integer", "description": "Max results (default 20)."}
            },
            "required": ["query"]
        }
    },
    {
        ...
        "description": (
            "Search ORCID for academic and research professionals. "
            "CC0 open data — zero IP risk. Use for academic institution or research ICPs."
        ),
        "parameters": {
            "type": "object",
            "properties": {
                "query": {"type": "string", "description": "Research topic or institution keyword."},
    ...
    },
    {
        ...
        "description": (
            "Fetch Glassdoor blog RSS for public company culture and hiring signals. "
            "Blog RSS only — no profile scraping."
        ),
        "parameters": {
            "type": "object",
            "properties": {
                "query": {"type": "string", "description": "Company or topic keyword."},
                "limit": {"type": "integer", "description": "Max results (default 15)."}
            },
            "required": ["query"]
        }
...
# ---------------------------------------------------------------------------
# Cross-reference source implementations (Layer 1 diversity)
# ---------------------------------------------------------------------------
...
def _search_wellfound_jobs(query: str, limit: int = 20) -> dict[str, Any]:
    ...
    results = search_wellfound_jobs(keywords=[query])
    return {"ok": True, "results": _slice_results(results, limit)}
...
def _search_crunchbase_news(query: str, limit: int = 20) -> dict[str, Any]:
    ...
    results = search_crunchbase_news(keywords=[query])
    return {"ok": True, "results": _slice_results(results, limit)}
...
def _scrape_team_page(company_name: str, domain: str, limit: int = 10) -> dict[str, Any]:
    ...
    return {"ok": True, "company": company_name, "domain": domain, "people": results, "count": len(results)}
...
def _search_conference_speakers(query: str, limit: int = 20) -> dict[str, Any]:
    ...
    results = search_conference_speakers(keywords=[query])
    return {"ok": True, "results": _slice_results(results, limit)}
...
def _search_orcid_profiles(query: str, limit: int = 15) -> dict[str, Any]:
    ...
    results = search_orcid_profiles(keywords=[query])
    return {"ok": True, "results": _slice_results(results, limit)}
...
def _search_glassdoor_signals(query: str, limit: int = 15) -> dict[str, Any]:
    ...
    results = search_glassdoor_news(keywords=[query])
    return {"ok": True, "results": _slice_results(results, limit)}
...
def dispatch_tool(name: str, arguments: dict[str, Any]) -> dict[str, Any]:
    handlers = {
        ...
        # cross-reference / Layer 1 diversity
